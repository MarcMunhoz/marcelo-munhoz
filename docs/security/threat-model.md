...
| High | Markdown, CMS fields, identity data, or media URLs become executable or unsafe browser content | Attacker-controlled or compromised upstream content reaches a DOM/navigation sink | Stored XSS, phishing navigation, or policy bypass | Article bodies cross a shared Markdown boundary that escapes raw HTML, constrains URLs, sanitizes generated HTML, and emits separately validated structured video blocks; titles retain text assignment or Vue interpolation; CSP remains defense in depth | Preserve the sanitizer allowlist, exact media canonicalization, Vue-owned iframe attributes, and hostile source-to-sink regressions whenever renderers or policy change | `app/src/utils/articleContent.js`, `app/src/components/ArticleContent.vue`, `app/src/components/BlogArticle.vue`, `app/src/pages/AdminArticleEditor.vue`, `app/netlify.toml` |
