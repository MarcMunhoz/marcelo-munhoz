...
- [ ] 3.5 Convert administrative SFCs to `<script setup>`: `AdminTags`, `AdminProfile`, and `Admin`, preserving sessions, authorization, native dialogs, forms, timers, and responsive behavior.
- [ ] 3.6 Convert `AdminArticleEditor` to `<script setup>` in isolation, preserving props/state, Contentful and Cloudinary workflows, tags, template refs, `nextTick`, unsaved-change detection, and the route-leave guard.
- [ ] 3.7 Verify all 15 SFCs contain `<script setup>` and no Options API remnants; run targeted tests per wave plus final lint, complete tests, production build, and public/admin route smoke checks in the container; document results.

## 4. ESLint Tooling

- [ ] 4.1 Upgrade ESLint and `eslint-plugin-vue` to v10-compatible releases.
- [ ] 4.2 Resolve flat-config, rule, parser, or plugin compatibility issues without broad suppressions.
- [ ] 4.3 Run lint and the complete test suite after the ESLint group.

## 5. Tailwind CSS

- [ ] 5.1 Review and decide whether the Tailwind v4 browser baseline is acceptable for this project.
- [ ] 5.2 If accepted, migrate Tailwind/PostCSS integration and configuration using the v4-compatible plugin path.
- [ ] 5.3 Compare representative public/admin layouts and document any browser-support or styling changes.

## 6. Final Validation

- [ ] 6.1 Run clean container install and verify lockfile consistency and npm audit results.
- [ ] 6.2 Run lint, all tests, production build, and runtime startup in the container.
- [ ] 6.3 Review the final diff and document upgraded, deferred, and reverted majors with evidence.
