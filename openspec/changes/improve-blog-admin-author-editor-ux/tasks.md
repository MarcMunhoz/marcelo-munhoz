...
- [ ] 1.1 Replace the rough authenticated user dropdown with a compact account menu showing display name, role, signed-in state, and sign-out.
- [ ] 1.2 Ensure the account menu renders cleanly without a profile photo and works for owner, writer, and local preview sessions.
- [ ] 1.3 Add sign-out confirmation and verify the admin returns to the unauthenticated state after logout.

## 2. Author Identity Mapping

- [ ] 2.1 Audit the current Netlify Identity user data, Contentful Author schema, and existing owner fallback mapping without reading local secret files.
- [ ] 2.2 Define and implement the trusted Identity-to-Author mapping field or fallback strategy.
- [ ] 2.3 Update article list and editor author resolution to use the trusted mapping consistently.
- [ ] 2.4 Add tests for resolved author, missing author, and legacy owner fallback states.

## 3. Author Profile Workflow

- [ ] 3.1 Add authenticated admin API endpoints to read and update the current user's Contentful Author profile.
- [ ] 3.2 Build an admin author profile page for public fields such as name, biography, optional photo, and author slug.
- [ ] 3.3 Keep Netlify Identity account fields separate from Contentful public author profile edits.
- [ ] 3.4 Add user-safe unresolved-profile and save-error states.

## 4. Public Author Information

- [ ] 4.1 Make article byline author names link to public author information when an author is resolved.
- [ ] 4.2 Build the public author information view or section with name, biography, optional photo, and authored article context where available.
- [ ] 4.3 Ensure public author rendering never exposes Identity e-mail, roles, invite state, or internal account identifiers.
- [ ] 4.4 Add tests or fixtures for byline links and author pages with and without photos.

## 5. Dedicated Article Editor Pages

- [ ] 5.1 Add route-level article creation and editing surfaces such as `/admin/articles/new` and `/admin/articles/:entryId/edit`.
- [ ] 5.2 Move the current article form workflow from the dashboard surface into the focused editor pages.
- [ ] 5.3 Preserve dashboard list, queue, and filter behavior as navigation into the editor pages.
- [ ] 5.4 Add unsaved-change protection and coherent return navigation from editor pages.
