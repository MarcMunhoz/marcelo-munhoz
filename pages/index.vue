<template>
  <div class="container-fluid mt-4">
    ...
    <p class="my-4 display-4 text-center">
      Quase 2 décadas trabalhando com web. HTML, CSS, JS e derivados.
    </p>

    <p class="my-4 display-4 text-center">
      Artes, games, literatura e cinema... Amo!
    </p>
    ...
    <div class="projects mt-5 border-top pt-4">
      ...
      <ul class="list-unstyled d-flex justify-content-around mt-5 mx-auto w-50">
        <li v-for="project in ProjectsList" :key="project.index" class="p-2">
          <a :href="project.projectUrl" class="font-weight-bold text-primary">{{ project.projectName }}</a>
...
export default {
  ...
  methods: {
    ...
    sortSocialNetwork() {
      return this.SocialNetwork.slice().sort((a, b) => {
        return a.nameAccount.localeCompare(b.nameAccount);
      });
    },
    filteredSocialList() {
      return this.sortSocialNetwork().filter((social) => social.useItOn.includes("where"));
    },
    ...
    cloudinaryImg: function(imgPath, imgName, imgWidth) {
      return this.$cloudinary.image.url(`${imgPath}/${imgName}`, { crop: "fit", width: imgWidth, transformation: [{ if: "w_lte_1875" }, { effect: "art:audrey" }, { if: "end" }] });
    },
